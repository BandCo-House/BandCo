...
test('밴드 생성 서비스는 repository가 만든 응답을 그대로 반환한다', async () => {
  const repository = createBandsRepositoryStub();
  const service = new BandsService(repository);

  const result = await service.createBand({
    name: '합주하자',
    description: '주 1회 합주하는 밴드입니다.',
    visibility: true,
  });

  assert.equal(result.band.id, 'band-created-001');
  assert.equal(result.band.name, '합주하자');
  assert.equal(result.band.inviteCode, '7KQ2M9');
});
