...
| 중간 | `src/modules/users/repositoreis/user.prisma-repository.spec.ts`, `src/modules/notifications/repositories/notifications.prisma-repository.spec.ts`, `src/modules/notifications/notifications.service.spec.ts` | 3개 spec의 `beforeEach`가 `Test.createTestingModule`로 의존성을 주입해 "Backend 필수 규칙"(auth 제외 TestingModule 금지)과 충돌. Repository/Service 생성자가 단순해 `new UsersPrismaRepository(mockPrisma)` 형태의 수동 주입으로 바꾸는 것 자체는 파일당 3~4줄이지만, 3개 파일에 걸쳐 있어 한 번에 정리하는 별도 PR로 분리 필요. `bandspaces.service.spec.ts`는 B7 수정(2026-09-06)에서 전 메서드 시그니처가 바뀌어 손으로 만든 Stub 패턴으로 전환 완료. | PR #149(회원가입 오류 수정) CodeRabbit 리뷰에서 `user.prisma-repository.spec.ts` 지적 — 해당 `beforeEach`는 이번 diff에서 건드리지 않은 기존 코드라 보류 |
| 낮음 | `src/modules/bandspaces/dto/update-band-space.dto.ts`, `src/modules/bandspaces/dto/update-bandspace-member-role.dto.ts` | 두 요청 DTO에 `@ApiProperty`/`@ApiPropertyOptional`이 없어 Swagger 데코레이터 의무화 규칙과 충돌. B7 수정(2026-09-06)은 컨트롤러·서비스·리포지토리만 대상이라 DTO는 손대지 않음. 별도 정리 필요. | B7(합주 공간 요청자 밴드 멤버 전환) 설계 중 발견 |
