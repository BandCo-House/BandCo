...
| 타이포그래피    | 크기·굵기는 `index.css`의 `typo-*` 유틸로만 지정한다. `text-sm`/`font-bold` 같은 Tailwind 기본 유틸을 `typo-*`와 같이 쓰지 않는다 — 병합 순서가 승자를 정해 의도와 다른 크기로 렌더된다. 역할별 배정: **32** 히어로 · **24** 페이지·모달·상세 제목 · **18** 섹션 제목·폼 라벨·시트/다이얼로그 제목 · **16** 본문·버튼 · **14** 보조 본문·빈 상태 안내문구·도움말·에러 메시지 · **12** 캡션·태그·메타. 안내문구를 12로 내리지 않는다. |
| 텍스트 색       | `text-grey-*`, `text-primary`, `text-destructive` 등 토큰만 쓴다. hex 리터럴(`text-[#9D9D9F]`)과 Tailwind 기본 팔레트(`slate`/`violet`/`gray`)는 쓰지 않는다.                                                                                                                                                                                                                                                                        |
