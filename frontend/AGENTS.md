...
| 모달 크기       | `AppDialogContent`는 `size`로 폭이 갈린다. 스크롤이 생길 수 있는 폼·목록·검색은 `size="full"`(좌우 여백 없이 앱 셸 폭 648), 한 화면에 들어가고 늘어날 일이 없는 확인창은 기본값 `compact`(양옆 16px). **헷갈리면 `full`** — compact인데 내용이 늘면 좁은 폰에서 스크롤과 여백이 겹쳐 두 번 조인다. |
| 모달 안 경고    | 경고 문구를 `absolute`로 띄우지 않는다. 레이아웃은 안 밀지만 스크롤 컨테이너의 overflow에는 잡혀, 문구는 안 보이는데 모달에만 스크롤이 생긴다.                                                                                                                                                     |
