...
describe('Input', () => {
  it('roundedFull variant가 지정되면 입력 필드는 둥근 테두리와 secondary focus 스타일을 가져야 한다', () => {
    ...
    expect(input).toHaveClass('hover:outline-secondary');
    expect(input).toHaveClass('focus-visible:outline-secondary');
    expect(input).not.toHaveClass('outline-none');
  });

  it('underline variant가 지정되면 입력 필드는 밑줄 테두리와 secondary focus 스타일을 가져야 한다', () => {
    ...
    expect(input).toHaveClass('hover:border-secondary');
    expect(input).toHaveClass('focus-visible:border-secondary');
  });
});
