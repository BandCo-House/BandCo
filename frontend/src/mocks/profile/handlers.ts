  ...
  http.patch(
    ...
    async ({ params, request }) => {
      ...
      if (body.profile) {
        ...
        // 요청은 profile.profileMusic으로 받지만 응답에서는 최상위 profileMusic으로 돌려준다 (백엔드 계약)
        if (body.profile.profileMusic !== undefined) {
          current.profileMusic = body.profile.profileMusic;
        }
      }
