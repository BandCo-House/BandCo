...
export const createBand = (
  ...
  apiPost<{ band: BandCreateResponse; items?: BandCreateResponse }>(
    '/bands',
    data,
  ).then((response) => {
    const rawBand = response.band ?? response.items ?? response;
    return bandCreateResponseSchema.parse(rawBand);
  });
